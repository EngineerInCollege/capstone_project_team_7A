...
// rgb helper
float *load_rgb_normalized_128(const char *image_path) {
    int w,h,comp;
    unsigned char *img = stbi_load(image_path, &w, &h, &comp, 3); // RGB
    if (!img) return NULL;

    // resize to 128x128 RGB
    unsigned char *tmp = malloc(128 * 128 * 3);
    ...
    float *out = malloc(3 * 128 * 128 * sizeof(float));
    for (int i=0; i<128*128*3; ++i) {
        float v01 = tmp[i] / 255.0f;
        out[i] = (v01 - 0.5f) / 0.5f;   // same as PyTorch Normalize
    }
...
static void center_crop_rgb_square(const uint8_t *src, int w, int h, int stride,
                                   uint8_t *dst, int side)
{
    ...
    for (int y = 0; y < side; ++y) {
        const uint8_t *row_in  = src + (y0 + y) * stride + x0 * 3;
        uint8_t       *row_out = dst + y * side * 3;
        memcpy(row_out, row_in, side * 3);
    }
}

static void resize_gray_128(const uint8_t *gray_in, int inW, int inH, float *out128){
    // resize full gray image to 128x128, then normalize to [-1,1]
    ...
    for (int i = 0; i < 128 * 128; ++i) {
        float v = tmp[i] / 255.0f;
        v = (v - 0.5f) / 0.5f;  // match PyTorch Normalize(mean=0.5, std=0.5)
        out128[i] = v;
    }
    free(tmp);
}
